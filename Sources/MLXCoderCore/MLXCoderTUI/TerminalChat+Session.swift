...
public extension TerminalChat {
    ...
    public func currentSessionConfiguration() async -> AgentCoreSessionConfiguration {
    ...
    public func selectedAllowedToolNames() async -> Set<String> {
        ...
        if selectedToolGroups.contains(.xcode) || selectedToolGroups.contains(.figma) {
            ...
            let mcpDescriptors = await sessionRunner.mcpToolDescriptors(
    ...
    @discardableResult
    public func updateCurrentSessionToolOptions() async -> Set<String> {
        let allowedToolNames = await selectedAllowedToolNames()
        do {
